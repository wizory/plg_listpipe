...
//        return Listpipe::OK;
    ...
    public function getContent($request) {
        // get a random post
        $article = $this->cms->getRandomPost();

        // fix an old typo
        $title = str_replace('Wilisonville', 'Wilsonville', $article['title']);
        $body = str_replace('Wilisonville', 'Wilsonville', $article['introtext']);
        // TODO make above functionality externalized?  Need variable length config element with pairs of search/replace

        return $title . Listpipe::DELIMITER . $body . Listpipe::DELIMITER . $article['cat_title'];
    }
    ...
    public function processContent($content) {
        ...
        $publish_timestamp = strtotime("+ ".rand(0,5)." seconds");
